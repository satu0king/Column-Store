...
namespace Parser {
    ...
    public:
        ...
        std::vector<ColumnStore::Column>& get_columns() { return columns; }
        ColumnStore::Column& operator[](int i) {
            if (i >= columns.size()) {
                throw runtime_error("Index out of Bounds in Table:" + table_name);
            }
            return columns[i];
        }
        ColumnStore::Column& get_column(std::string name) { return (*this)[name]; }
        ColumnStore::Column& operator[](std::string name) {
