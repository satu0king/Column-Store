...
void create_dummy_data(string file_name) {
    ...
    for(int i = 1; i <= num_departments; i++) {
        to_write += ("(" + to_string(i) + ",'department_" + to_string(i) + "'),");
    }
    ...
    to_write = "INSERT INTO EMPLOYEE (id, name, age, salary, gender, dept_id, city_id) VALUES ";
    ...
    for(int i = 1; i <= num_employees; i++) {
        ...
        to_write += ("," + to_string(age) + "," + to_string(salary) + "," + to_string(gender) + "," + to_string(dept_id) + "," + to_string(city_id));
        to_write += "),";
    }
